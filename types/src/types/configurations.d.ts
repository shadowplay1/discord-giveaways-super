...
export declare type IGiveawaysConfiguration<TDatabaseType extends DatabaseType> = {
    /**
...
export declare type IGiveawayButtons = Record<'joinGiveawayButton' | 'rerollButton', Partial<IGiveawayJoinButtonOptions>> & {
...
export declare type IGiveawayEmbedOptions = Partial<Record<'messageContent' | 'title' | 'titleIcon' | 'titleIconURL' | 'description' | 'footer' | 'footerIcon' | 'thumbnailURL' | 'imageURL', string> & {
    color: ColorResolvable;
}>;
export declare type IGiveawaysConfigurationWithoutDatabase = Omit<Required<IGiveawaysConfiguration<DatabaseType.JSON>>, 'database' | 'connection'>;
export declare type DatabaseConnectionOptions<TDatabase extends DatabaseType> = TDatabase extends DatabaseType.JSON ? Partial<IJSONDatabseConfiguration> : TDatabase extends DatabaseType.ENMAP ? EnmapOptions<any, any> : TDatabase extends DatabaseType.MONGODB ? IMongoConnectionOptions : never;
export declare type Database<TDatabase extends DatabaseType> = TDatabase extends DatabaseType.JSON ? null : TDatabase extends DatabaseType.ENMAP ? Enmap<string, IDatabaseStructure> : TDatabase extends DatabaseType.MONGODB ? Mongo<IDatabaseStructure> : never;
