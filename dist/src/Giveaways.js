...
class Giveaways extends Emitter_1.Emitter {
                    ...
                    if (giveaway) {
                        ...
                        if (giveaway.participantsFilter?.restrictedMembers?.length) {
                            ...
                            if (restrictedMembers.includes(interaction.user.id)) {
                                if (!TypedObject_1.TypedObject.keys(memberRestrictionMessage).length) {
                                ...
                                interaction.reply({
                                    ...
                                    embeds: TypedObject_1.TypedObject.keys(memberRestrictionMessage).length == 1 &&
                        ...
                        if (giveaway.participantsFilter?.requiredRoles?.length) {
                            ...
                            if (!memberHasAtLeastOneRequiredRole) {
                                if (!TypedObject_1.TypedObject.keys(hasNoRequiredRolesMessage).length) {
                                ...
                                interaction.reply({
                                    ...
                                    embeds: TypedObject_1.TypedObject.keys(hasNoRequiredRolesMessage).length == 1 &&
                                        hasNoRequiredRolesMessage?.messageContent
                        ...
                        if (giveaway.participantsFilter?.restrictedRoles?.length) {
                            ...
                            for (const restrictedRole of restrictedRoles) {
                                ...
                                if (memberHasRestrictedRole) {
                                    if (!TypedObject_1.TypedObject.keys(hasRestrictedRolesMessage).length) {
                                    ...
                                    interaction.reply({
                                        ...
                                        embeds: TypedObject_1.TypedObject.keys(hasRestrictedRolesMessage).length == 1 &&
                                            hasRestrictedRolesMessage?.messageContent
                        ...
                        if (!isUserJoined) {
                            ...
                            if (!TypedObject_1.TypedObject.keys(giveawayJoinMessage).length) {
                                giveawayJoinMessage.messageContent = 'You have joined the giveaway!';
                            }
                            interaction.reply({
                                content: giveawayJoinMessage?.messageContent,
                                embeds: TypedObject_1.TypedObject.keys(giveawayJoinMessage).length == 1 &&
                    ...
                    if (giveaway) {
                        if (interaction.user.id !== giveaway?.host.id) {
                            ...
                            interaction.reply({
                                ...
                                embeds: TypedObject_1.TypedObject.keys(onlyHostCanReroll).length == 1 && rerollErroredMessageContent ? [] : [errorEmbed],
                                ephemeral: true
                            }).catch((err) => {
                                // catching the "unknown interaction" error
                                // while still sending the responce on the button click somehow
                            ...
                            interaction.reply({
                                ...
                                embeds: TypedObject_1.TypedObject.keys(rerollSuccess).length == 1 && rerollSuccessfulMessageCreate ? [] : [successEmbed],
                                ephemeral: true
                            }).catch((err) => {
                                // catching the "unknown interaction" error
                                // while still sending the responce on the button click somehow
    ...
    async start(giveawayOptions) {
        ...
        const message = await channel.send({
            ...
            embeds: TypedObject_1.TypedObject.keys(startEmbedStrings).length == 1 && startEmbedStrings?.messageContent ? [] : [giveawayEmbed],
